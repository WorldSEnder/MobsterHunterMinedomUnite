...
public class MHItemChestContainer extends Container {
	...
	public MHItemChestContainer(EntityPlayer thePlayer, int invType, EntityMHVillager interactPerson, int page)
	{
		this.interactPerson = interactPerson;
		this.slots = new Slot[1000];
		String ident = PlayerHunterStats.identifier;
		PlayerHunterStats toBind = (PlayerHunterStats) thePlayer.getExtendedProperties(ident);
		...
		{
			throw new IllegalArgumentException("Select one of the static final variables in this class and nothing else.");
		}
		...
		if(inv instanceof InventoryMHItemChest)
		{
			for(int pageOn = 0; pageOn<10; pageOn++)
			{
				for(int i = 0; i<10; i++)
				{
					for(int j = 0; j<10; j++)
					{
						this.slots[100*pageOn+10*i+j] = this.addSlotToContainer(new SlotMHItem(inv, pageOn*100+10*i+j, pageOn*1000+17+j*20, 0+i*18, isWeaponInv));
					}
				}
			}
			this.currentPage = 0;
	...
	public boolean canInteractWith(EntityPlayer entityplayer) {
		return entityplayer.getDistanceSq(this.interactPerson.posX, this.interactPerson.posY, this.interactPerson.posZ) < 64;
	}
	...
	public void setPageOn(int page)
	{
		...
		{
			this.slots[i].xDisplayPosition = (i/100-page)*1000+17+i%10*20;
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack())
		{
			...
			if (par2 < 1000)
			{
				if (!this.mergeItemStack(itemstack1, 1000, this.inventorySlots.size(), false))
				{
					return null;
				}
			}
			else if (!this.slots[0].isItemValid(itemstack)||!this.mergeItemStack(itemstack1, 0, 1000, false))
			{
				return null;
			}
